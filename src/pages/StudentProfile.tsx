...
import { ArrowLeft, Wallet, User, CreditCard, Loader2, Phone, MessageSquare, Edit, Trash, Download, Calculator, Calendar, ChevronLeft, ChevronRight, Edit2, Trash2, Upload, AlertCircle } from "lucide-react";
...
interface Student { id: string; name: string; phone: string; father_name: string; father_phone: string; branch: string; enrollment_number: string; registration_date: string; due_date: string; photo_url?: string; status: 'active' | 'fees_due' | 'inactive'; fees_paid: number; fees_due: number; }
...
const EditStudentDialog = ({ isOpen, onClose, student, onSave }: { isOpen: boolean, onClose: () => void, student: Student | null, onSave: (updatedStudent: Partial<Student>, newPhoto: File | null) => void }) => { const [formData, setFormData] = useState<Partial<Student>>({}); const [newPhotoFile, setNewPhotoFile] = useState<File | null>(null); const [photoPreview, setPhotoPreview] = useState<string | null>(null); useEffect(() => { if (student) { setFormData(student); setPhotoPreview(student.photo_url || null); } }, [student, isOpen]); const handleChange = (field: keyof Student, value: string | Date | null) => { let finalValue = value; if (value instanceof Date) { finalValue = format(value, 'yyyy-MM-dd'); } setFormData(prev => ({ ...prev, [field]: finalValue })); }; const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if (file) { setNewPhotoFile(file); setPhotoPreview(URL.createObjectURL(file)); } }; const handleSaveChanges = () => { onSave(formData, newPhotoFile); onClose(); }; if (!student) return null; return ( <Dialog open={isOpen} onOpenChange={onClose}><DialogContent className="sm:max-w-[600px]"><DialogHeader><DialogTitle>Edit Student Details</DialogTitle></DialogHeader><div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-4 max-h-[70vh] overflow-y-auto pr-2"><div className="space-y-2"><Label>Full Name</Label><Input value={formData.name || ''} onChange={e => handleChange('name', e.target.value)} /></div><div className="space-y-2"><Label>Enrollment Number</Label><Input value={formData.enrollment_number || ''} onChange={e => handleChange('enrollment_number', e.target.value)} /></div><div className="space-y-2"><Label>Phone</Label><Input value={formData.phone || ''} onChange={e => handleChange('phone', e.target.value)} /></div><div className="space-y-2"><Label>Father's Phone</Label><Input value={formData.father_phone || ''} onChange={e => handleChange('father_phone', e.target.value)} /></div><div className="space-y-2"><Label>Registration Date</Label><Popover><PopoverTrigger asChild><Button variant="outline" className="w-full justify-start text-left font-normal">{formData.registration_date ? format(new Date(formData.registration_date), "PPP") : "Select date"}</Button></PopoverTrigger><PopoverContent className="w-auto p-0"><CalendarComponent mode="single" selected={formData.registration_date ? new Date(formData.registration_date) : undefined} onSelect={(d) => d && handleChange('registration_date', d)} /></PopoverContent></Popover></div><div className="space-y-2"><Label>Next Fee Due Date</Label><Popover><PopoverTrigger asChild><Button variant="outline" className="w-full justify-start text-left font-normal">{formData.due_date ? format(new Date(formData.due_date), "PPP") : "Select date"}</Button></PopoverTrigger><PopoverContent className="w-auto p-0"><CalendarComponent mode="single" selected={formData.due_date ? new Date(formData.due_date) : undefined} onSelect={(d) => d && handleChange('due_date', d)} /></PopoverContent></Popover></div><div className="space-y-2"><Label>Status</Label><Select value={formData.status} onValueChange={(v: Student['status']) => handleChange('status', v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="active">Active</SelectItem><SelectItem value="fees_due">Fees Due</SelectItem><SelectItem value="inactive">Inactive</SelectItem></SelectContent></Select></div><div className="space-y-2 col-span-full"><Label>Profile Photo</Label><div className="flex items-center gap-4"><img src={photoPreview || `https://ui-avatars.com/api/?name=${student.name}&background=random`} alt="preview" className="w-16 h-16 rounded-full object-cover"/><Button type="button" variant="outline" size="sm" className="relative"><Upload className="w-4 h-4 mr-2" /> Change Photo<input type="file" accept="image/*" onChange={handlePhotoChange} className="absolute inset-0 opacity-0 cursor-pointer" /></Button></div></div></div><DialogFooter><Button variant="outline" onClick={onClose}>Cancel</Button><Button onClick={handleSaveChanges}>Save Changes</Button></DialogFooter></DialogContent></Dialog> );};
...
const ReminderActions = ({ student, formatAmount }: { student: Student, formatAmount: (amount: number) => string }) => { const whatsappUrl = useMemo(() => { let phoneNumber = student.phone.replace(/[^0-9]/g, ''); if (phoneNumber.length === 10) phoneNumber = '91' + phoneNumber; let message: string; if (student.fees_due > 0) { const amountDue = formatAmount(student.fees_due); message = `Hi ${student.name}, this is a friendly reminder from the mess management. Your fee of ${amountDue} is pending. Please pay at your earliest convenience. Thank you!`; } else { message = `Hi ${student.name}, this is a message from the mess management.`; } return `https://wa.me/${phoneNumber}?text=${encodeURIComponent(message)}`; }, [student.name, student.phone, student.fees_due, formatAmount]); return ( <Card className="shadow-card bg-gradient-card"><CardHeader><CardTitle className="flex items-center gap-2"><AlertCircle className="w-5 h-5 text-primary" /> Quick Actions</CardTitle></CardHeader><CardContent className="space-y-3"><p className="text-sm text-muted-foreground">Quickly contact the student.</p><Button asChild className="w-full"><a href={`tel:${student.phone}`}><Phone className="w-4 h-4 mr-2" /> Call Student</a></Button><Button variant="outline" className="w-full" onClick={() => window.open(whatsappUrl, '_blank')}><MessageSquare className="w-4 h-4 mr-2" /> Send WhatsApp Message</Button></CardContent></Card> );};
...
const StudentProfile = () => {
  ...
  const handleUpdateStudent = async (updatedData: Partial<Student>, newPhotoFile: File | null) => {
    ...
    try {
        ...
        const { id, created_at, fees_paid, fees_due, ...updatableData } = finalUpdateData; // created_at, fees_paid, fees_due should not be directly editable
  ...
  const handleDeleteStudent = async () => { if (!student) return; if (!window.confirm(`Are you sure you want to delete ${student.name}'s profile? All associated data will be lost.`)) return; try { const { error } = await supabase.from('students').delete().eq('id', student.id); if (error) throw error; toast.success("Student profile deleted permanently."); navigate('/profiles'); } catch (error: any) { toast.error("Failed to delete student: " + error.message); } };
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="shadow-card bg-gradient-card">
            ...
            <CardContent className="space-y-4">
              <SummaryCards cards={[{ label: "Total Paid", value: formatAmount(student.fees_paid), tone: "success" }, { label: "Amount Due", value: formatAmount(student.fees_due), tone: "danger" }, { label: "Transactions", value: `${transactions.length}` },]} />
          ...
          <Card className="shadow-card bg-gradient-card">
            ...
            <CardContent className="text-center">
              ...
              <div className="pt-4 mt-4 border-t space-y-2 text-sm text-left">
                  <div className="flex justify-between"><span>Status:</span> <Badge variant={student.status === 'active' ? 'default' : 'destructive'} className={student.status === 'active' ? 'bg-green-100 text-green-700' : ''}>{student.status.replace('_', ' ').toUpperCase()}</Badge></div>
              </div>
            </CardContent>
          </Card>
          <ReminderActions student={student} formatAmount={formatAmount} />
